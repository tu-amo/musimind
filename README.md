...
To use this module, create a [simulation](README.md##simulation) for an array of [nodes](README.md##simulation_nodes), and compose the desired [forces](README.md##simulation_force). Then [listen](README.md##simulation_on) for tick events to render the nodes as they update in your preferred graphics system, such as Canvas or SVG.
...
Creates a new simulation with the specified array of [*nodes*](README.md.md##simulation_nodes) and no [forces](README.md.md##simulation_force). If *nodes* is not specified, it defaults to the empty array. The simulator [starts](README.md##simulation_restart) automatically; use [*simulation*.on](README.md.md##simulation_on) to listen for tick events as the simulation runs. If you wish to run the simulation manually instead, call [*simulation*.stop](README.md##simulation_stop), and then call [*simulation*.tick](README.md##simulation_tick) as desired.
...
Restarts the simulation’s internal timer and returns the simulation. In conjunction with [*simulation*.alphaTarget](README.md##simulation_alphaTarget) or [*simulation*.alpha](README.md##simulation_alpha), this method can be used to “reheat” the simulation during interaction, such as when dragging a node, or to resume the simulation after temporarily pausing it with [*simulation*.stop](README.md.md##simulation_stop).
...
Stops the simulation’s internal timer, if it is running, and returns the simulation. If the timer is already stopped, this method does nothing. This method is useful for running the simulation manually; see [*simulation*.tick](README.md.md##simulation_tick).
...
For each iteration, it increments the current [*alpha*](README.md.md##simulation_alpha) by ([*alphaTarget*](README.md.md##simulation_alphaTarget) - *alpha*) × [*alphaDecay*](README.md##simulation_alphaDecay); then invokes each registered [force](README.md.md.md##simulation_force), passing the new *alpha*; then decrements each [node](README.md.md.md##simulation_nodes)’s velocity by *velocity* × [*velocityDecay*](README.md##simulation_velocityDecay); lastly increments each node’s position by *velocity*.

This method does not dispatch [events](README.md.md.md##simulation_on); events are only dispatched by the internal timer when the simulation is started automatically upon [creation](README.md##forceSimulation) or by calling [*simulation*.restart](README.md.md##simulation_restart). The natural number of ticks when the simulation is started is ⌈*log*([*alphaMin*](README.md##simulation_alphaMin)) / *log*(1 - [*alphaDecay*](README.md.md##simulation_alphaDecay))⌉; by default, this is 300.

This method can be used in conjunction with [*simulation*.stop](README.md.md.md##simulation_stop) to compute a [static force layout](https://bl.ocks.org/mbostock/1667139). For large graphs, static layouts should be computed [in a web worker](https://bl.ocks.org/mbostock/01ab2e85e8727d6529d20391c0fd9a16) to avoid freezing the user interface.
...
If *nodes* is specified, sets the simulation’s nodes to the specified array of objects, initializing their positions and velocities if necessary, and then [re-initializes](README.md##force_initialize) any bound [forces](README.md.md.md.md##simulation_force); returns the simulation. If *nodes* is not specified, returns the simulation’s array of nodes as specified to the [constructor](README.md.md##forceSimulation).
...
At the end of each [tick](README.md.md.md##simulation_tick), after the application of any forces, a node with a defined *node*.fx has *node*.x reset to this value and *node*.vx set to zero; likewise, a node with a defined *node*.fy has *node*.y reset to this value and *node*.vy set to zero. To unfix a node that was previously fixed, set *node*.fx and *node*.fy to null, or delete these properties.
...
*alpha* is roughly analogous to temperature in [simulated annealing](https://en.wikipedia.org/wiki/Simulated_annealing#Overview). It decreases over time as the simulation “cools down”. When *alpha* reaches *alphaMin*, the simulation stops; see [*simulation*.restart](README.md.md.md##simulation_restart).
...
If *min* is specified, sets the minimum *alpha* to the specified number in the range [0,1] and returns this simulation. If *min* is not specified, returns the current minimum *alpha* value, which defaults to 0.001. The simulation’s internal timer stops when the current [*alpha*](README.md.md##simulation_alpha) is less than the minimum *alpha*. The default [alpha decay rate](README.md.md.md##simulation_alphaDecay) of ~0.0228 corresponds to 300 iterations.
...
If *decay* is specified, sets the [*alpha*](README.md.md##simulation_alpha) decay rate to the specified number in the range [0,1] and returns this simulation. If *decay* is not specified, returns the current *alpha* decay rate, which defaults to 0.0228… = 1 - *pow*(0.001, 1 / 300) where 0.001 is the default [minimum *alpha*](README.md.md##simulation_alphaMin).

The alpha decay rate determines how quickly the current alpha interpolates towards the desired [target *alpha*](README.md.md.md##simulation_alphaTarget); since the default target *alpha* is zero, by default this controls how quickly the simulation cools. Higher decay rates cause the simulation to stabilize more quickly, but risk getting stuck in a local minimum; lower values cause the simulation to take longer to run, but typically converge on a better layout. To have the simulation run forever at the current *alpha*, set the *decay* rate to zero; alternatively, set a [target *alpha*](README.md.md.md##simulation_alphaTarget) greater than the [minimum *alpha*](README.md.md.md##simulation_alphaMin).
...
If *target* is specified, sets the current target [*alpha*](README.md.md##simulation_alpha) to the specified number in the range [0,1] and returns this simulation. If *target* is not specified, returns the current target alpha value, which defaults to 0.
...
If *decay* is specified, sets the velocity decay factor to the specified number in the range [0,1] and returns this simulation. If *decay* is not specified, returns the current velocity decay factor, which defaults to 0.4. The decay factor is akin to atmospheric friction; after the application of any forces during a [tick](README.md.md.md##simulation_tick), each node’s velocity is multiplied by 1 - *decay*. As with lowering the [alpha decay rate](README.md.md.md.md##simulation_alphaDecay), less velocity decay may converge on a better solution, but risks numerical instabilities and oscillation.
...
If *force* is specified, assigns the [force](README.md.md##forces) for the specified *name* and returns this simulation. If *force* is not specified, returns the force with the specified name, or undefined if there is no such force. (By default, new simulations have no forces.) For example, to create a new simulation to layout a graph, you might say:
...
* `end` - after the simulation’s timer stops when *alpha* < [*alphaMin*](README.md.md.md.md##simulation_alphaMin).

Note that *tick* events are not dispatched when [*simulation*.tick](README.md.md##simulation_tick) is called manually; events are only dispatched by the internal timer and are intended for interactive rendering of the simulation. To affect the simulation, register [forces](README.md.md.md.md.md##simulation_force) instead of modifying nodes’ positions or velocities inside a tick event listener.
...
* [Centering](README.md##centering)
* [Collision](README.md##collision)
* [Links](README.md##links)
* [Many-Body](README.md##many-body)
* [Positioning](README.md##positioning)

Forces may optionally implement [*force*.initialize](README.md.md##force_initialize) to receive the simulation’s array of nodes.

<a name="_force" href="#_force">#</a> <i>force</i>(<i>alpha</i>) · [Source](https://github.com/d3/d3-force/blob/master/src/simulation.js)

Applies this force, optionally observing the specified *alpha*. Typically, the force is applied to the array of nodes previously passed to [*force*.initialize](README.md.md##force_initialize), however, some forces may apply to a subset of nodes, or behave differently. For example, [d3.forceLink](README.md.md##links) applies to the source and target of each link.
...
Supplies the array of *nodes* and *random* source to this force. This method is called when a force is bound to a simulation via [*simulation*.force](README.md.md.md.md.md.md##simulation_force) and when the simulation’s nodes change via [*simulation*.nodes](README.md.md.md.md##simulation_nodes). A force may perform necessary work during initialization, such as evaluating per-node parameters, to avoid repeatedly performing work during each application of the force.

#### Centering

The centering force translates nodes uniformly so that the mean position of all nodes (the center of mass if all nodes have equal weight) is at the given position ⟨[*x*](README.md##center_x),[*y*](README.md##center_y)⟩. This force modifies the positions of nodes on each application; it does not modify velocities, as doing so would typically cause the nodes to overshoot and oscillate around the desired center. This force helps keeps nodes in the center of the viewport, and unlike the [positioning force](README.md.md##positioning), it does not distort their relative positions.
...
Creates a new centering force with the specified [*x*-](README.md.md##center_x) and [*y*-](README.md.md##center_y) coordinates. If *x* and *y* are not specified, they default to ⟨0,0⟩.
...
Creates a new circle collision force with the specified [*radius*](README.md.md##collide_radius). If *radius* is not specified, it defaults to the constant one for all nodes.
...
The link force pushes linked nodes together or apart according to the desired [link distance](README.md##link_distance). The strength of the force is proportional to the difference between the linked nodes’ distance and the target distance, similar to a spring force.
...
If *links* is specified, sets the array of links associated with this force, recomputes the [distance](README.md.md##link_distance) and [strength](README.md##link_strength) parameters for each link, and returns this force. If *links* is not specified, returns the current array of links, which defaults to the empty array.

Each link is an object with the following properties:

* `source` - the link’s source node; see [*simulation*.nodes](README.md.md.md.md.md.md##simulation_nodes)
* `target` - the link’s target node; see [*simulation*.nodes](README.md.md.md.md.md.md##simulation_nodes)
* `index` - the zero-based index into *links*, assigned by this method

For convenience, a link’s source and target properties may be initialized using numeric or string identifiers rather than object references; see [*link*.id](README.md##link_id). When the link force is [initialized](README.md.md.md.md##force_initialize) (or re-initialized, as when the nodes or links change), any *link*.source or *link*.target property which is *not* an object is replaced by an object reference to the corresponding *node* with the given identifier.
...
The default id accessor allows each link’s source and target to be specified as a zero-based index into the [nodes](README.md.md.md.md.md.md.md.md##simulation_nodes) array. For example:
...
The id accessor is invoked for each node whenever the force is initialized, as when the [nodes](README.md.md.md.md.md.md.md.md##simulation_nodes) or [links](README.md##link_links) change, being passed the node and its zero-based index.
...
The distance accessor is invoked for each [link](README.md.md##link_links), being passed the *link* and its zero-based *index*. The resulting number is then stored internally, such that the distance of each link is only recomputed when the force is initialized or when this method is called with a new *distance*, and not on every application of the force.
...
The strength accessor is invoked for each [link](README.md.md##link_links), being passed the *link* and its zero-based *index*. The resulting number is then stored internally, such that the strength of each link is only recomputed when the force is initialized or when this method is called with a new *strength*, and not on every application of the force.
...
The many-body (or *n*-body) force applies mutually amongst all [nodes](README.md.md.md.md.md.md.md.md##simulation_nodes). It can be used to simulate gravity (attraction) if the [strength](README.md##manyBody_strength) is positive, or electrostatic charge (repulsion) if the strength is negative. This implementation uses quadtrees and the [Barnes–Hut approximation](https://en.wikipedia.org/wiki/Barnes–Hut_simulation) to greatly improve performance; the accuracy can be customized using the [theta](README.md##manyBody_theta) parameter.
...
The [*x*](README.md##forceX)- and [*y*](README.md##forceY)-positioning forces push nodes towards a desired position along the given dimension with a configurable strength. The [*radial*](README.md##forceRadial) force is similar, except it pushes nodes towards the closest point on a given circle. The strength of the force is proportional to the one-dimensional distance between the node’s position and the target position. While these forces can be used to position individual nodes, they are intended primarily for global forces that apply to all (or most) nodes.

<a name="forceX" href="#forceX">#</a> d3.<b>forceX</b>([<i>x</i>]) · [Source](https://github.com/d3/d3-force/blob/master/src/x.js)

Creates a new positioning force along the *x*-axis towards the given position [*x*](README.md##x_x). If *x* is not specified, it defaults to 0.
...
Creates a new positioning force along the *y*-axis towards the given position [*y*](README.md##y_y). If *y* is not specified, it defaults to 0.
...
If *strength* is specified, sets the strength accessor to the specified number or function, re-evaluates the strength accessor for each node, and returns this force. The *strength* determines how much to increment the node’s *y*-velocity: ([*y*](README.md##y_y) - *node*.y) × *strength*. For example, a value of 0.1 indicates that the node should move a tenth of the way from its current *y*-position to the target *y*-position with each application. Higher values moves nodes more quickly to the target position, often at the expense of other forces or constraints. A value outside the range [0,1] is not recommended.
...
Creates a new positioning force towards a circle of the specified [*radius*](README.md##radial_radius) centered at ⟨[*x*](README.md##radial_x),[*y*](README.md##radial_y)⟩. If *x* and *y* are not specified, they default to ⟨0,0⟩.
